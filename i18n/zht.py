...
class TraditionalChineseTranslation(BaseTranslation):
    ...
    def translations(self) -> dict:
        ...
        'web_search_button': '全網搜尋',
        'web_search_button_tooltip': '使用 Brave Search 檢索全網，並由 AI 決定繼續搜尋或彙總回答。請先在「設定 → 搜尋」中綁定 Brave API Key。',
        'web_search_section_title': '全網搜尋（Brave）',
        'web_search_section_subtitle': '在問詢中使用你自己的 Brave Search API Key 取得即時網頁結果',
        'web_search_section_description': '在問詢欄打開「全網搜尋」後，AI 可以檢索當前網路資訊。外掛會使用你在此綁定的金鑰呼叫 Brave Search API。請到 <a href="https://api-dashboard.search.brave.com/">Brave Search API 控制台</a> 申請金鑰。',
        ...
        'web_search_privacy_alert': '打開全網搜尋後，搜尋詞會傳送給 Brave Search（不會傳給你的 AI 供應商）。查詢中的書名等資訊對 Brave 可見。',
        ...
        'web_search_missing_key': '已打開全網搜尋，但尚未設定 Brave Search API Key。請打開「設定 → 搜尋」並貼上你的 Brave API Key。',
        'web_search_invalid_key': 'Brave Search 拒絕了該 API Key（HTTP {status}）。請在「設定 → 搜尋」中檢查金鑰和方案。',
