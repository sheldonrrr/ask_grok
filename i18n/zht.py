...
class TraditionalChineseTranslation(BaseTranslation):
    ...
    def translations(self) -> dict:
        ...
        'set_default_ai_after_add_message': '已成功新增「{0}」。是否將其設為預設 AI？',
        'copied': '已複製！',
        'pdf_exported': 'PDF已匯出！',
        ...
        'model_display_name_custom': '自定義 (OpenAI 相容)',
            'model_display_name_ollama': 'Ollama(本地)',
            'model_display_name_lmstudio': 'LM Studio(本地)',
            'model_display_name_koboldcpp': 'KoboldCpp(本地)',
            'custom_openai_compat_notice': '提示：自定義服務商使用 OpenAI Chat Completions 請求格式（/chat/completions）。請填寫相容的 Base URL 與模型名稱。部分本地服務可不填 API Key。',
        ...
        'web_search_button': '網頁搜尋',
        'web_search_button_tooltip': '使用 Brave Search 檢索全網，並由 AI 決定繼續搜尋或彙總回答。請先在「設定 → 一般」中綁定 Brave API Key。',
        'web_search_section_title': '網頁搜尋（Brave）',
        'web_search_section_subtitle': '在問詢中使用你自己的 Brave Search API Key 取得即時網頁結果',
        'web_search_section_description': '在問詢欄打開「網頁搜尋」後，AI 可以檢索當前網路資訊。外掛會使用你在此綁定的金鑰呼叫 Brave Search API。請到 <a href="https://api-dashboard.search.brave.com/">Brave Search API 控制台</a> 申請金鑰。',
        'web_search_api_key_label': 'Brave Search API Key',
        'web_search_api_key_placeholder': '貼上你的 Brave Subscription Token',
        'web_search_api_key_desc': '金鑰只保存在本機的外掛設定中，僅會傳送到 api.search.brave.com。',
        'web_search_privacy_alert': '打開網頁搜尋後，搜尋詞會傳送給 Brave Search（不會傳給你的 AI 供應商）。查詢中的書名等資訊對 Brave 可見。',
        'web_search_test_button': '測試',
        'web_search_test_success': 'Brave Search API 可用。測試查詢返回了 {count} 條結果。',
        'web_search_test_failed': 'Brave Search API 測試失敗：{error}',
        'web_search_missing_key': '已打開網頁搜尋，但尚未設定 Brave Search API Key。請打開「設定 → 一般」並貼上你的 Brave API Key。',
        'web_search_invalid_key': 'Brave Search 拒絕了該 API Key（HTTP {status}）。請在「設定 → 一般」中檢查金鑰和方案。',
        'web_search_rate_limited': 'Brave Search 已達到速率限制（HTTP 429）。請稍後再試，或檢查方案配額。',
        'web_search_request_failed': '網頁搜尋失敗（「{query}」）：{error}',
        'web_search_searching': '正在搜尋：{query}',
        'web_search_no_results': '沒有找到相關網頁：{query}',
        'web_search_heading': '網頁搜尋：{query}',
        'web_search_no_previous_results': '（尚無）',
        'web_search_log_planning': '正在規劃網頁搜尋（第 {round}/{max} 輪）…',
        'web_search_log_requested': 'AI 請求繼續搜尋：{queries}',
        'web_search_log_answering': 'AI 正在彙總回答…',
        'web_search_log_force_answer': '已達搜尋輪次上限，根據已有結果作答…',
