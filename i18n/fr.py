...
class FrenchTranslation(BaseTranslation):
    ...
    def translations(self) -> dict:
        return {
            ...
            'model_display_name_custom': 'Personnalisé (compatible OpenAI)',
            ...
            'custom_openai_compat_notice': 'Remarque : Custom utilise le format OpenAI Chat Completions (/chat/completions). Saisissez une Base URL et un nom de modèle compatibles. La clé API est facultative pour certains serveurs locaux.',
            'local_openai_compat_no_api_key_notice': 'Remarque : ce service local compatible OpenAI ne nécessite généralement pas de clé API. Démarrez le serveur local, puis actualisez la liste des modèles.',
            'lmstudio_no_api_key_notice': "Remarque : LM Studio utilise localement l'API compatible OpenAI et ne nécessite généralement pas de clé API.",
            'koboldcpp_no_api_key_notice': "Remarque : KoboldCpp utilise localement l'API compatible OpenAI et ne nécessite généralement pas de clé API.",
            'local_service_not_running': "Impossible de se connecter au service d'IA local. Vérifiez qu'il est démarré et que l'URL de base est correcte.",
