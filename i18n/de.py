...
class GermanTranslation(BaseTranslation):
    ...
    def translations(self) -> dict:
        return {
            ...
            'set_default_ai_after_add_message': '„{0}" wurde erfolgreich hinzugefügt. Möchten Sie sie als Standard-KI festlegen?',
            ...
            'model_display_name_custom': 'Benutzerdefiniert (OpenAI-kompatibel)',
            'model_display_name_ollama': 'Ollama(Lokal)',
            'model_display_name_lmstudio': 'LM Studio(Lokal)',
            'model_display_name_koboldcpp': 'KoboldCpp(Lokal)',
            'custom_openai_compat_notice': 'Hinweis: Custom nutzt das OpenAI Chat Completions-Format (/chat/completions). Geben Sie eine kompatible Base URL und einen Modellnamen ein. API-Key ist bei manchen lokalen Servern optional.',
            'local_openai_compat_no_api_key_notice': 'Hinweis: Dieser lokale OpenAI-kompatible Dienst benötigt in der Regel keinen API-Schlüssel. Starten Sie den lokalen Server und aktualisieren Sie dann die Modellliste.',
            'lmstudio_no_api_key_notice': 'Hinweis: LM Studio nutzt lokal die OpenAI-kompatible API und benötigt in der Regel keinen API-Schlüssel.',
            'koboldcpp_no_api_key_notice': 'Hinweis: KoboldCpp nutzt lokal die OpenAI-kompatible API und benötigt in der Regel keinen API-Schlüssel.',
            'local_service_not_running': 'Keine Verbindung zum lokalen KI-Dienst. Bitte prüfen Sie, ob er läuft und die Base URL korrekt ist.',
            ...
            'no_configured_ai_hint': 'Keine KI konfiguriert. Plugin kann nicht funktionieren. Bitte klicken Sie auf "KI hinzufügen", um einen KI-Anbieter hinzuzufügen.',
            'default_ai_label': 'Standard-KI:',
            'default_ai_tag': 'Standard',
            'ai_not_configured_cannot_set_default': 'Diese KI ist noch nicht konfiguriert. Bitte speichern Sie zuerst die Konfiguration.',
            ...
            'confirm_remove_ai': 'Sind Sie sicher, dass Sie {name} entfernen möchten? Dies löscht den API-Schlüssel und setzt die Konfiguration zurück.',
            ...
            'base_url_desc_kimi': 'Internationale Schlüssel verwenden https://api.moonshot.ai/v1; Schlüssel der China-Plattform verwenden https://api.moonshot.cn/v1. Nicht mischen.',
            'kimi_region_label': 'Plattform',
            'kimi_region_global': 'International',
            'kimi_region_china': 'China (Festland)',
            'kimi_base_url_readonly_tip': 'Die Basis-URL wird durch die gewählte Plattform bestimmt.',
            ...
            'web_search_button_tooltip': 'Search the web with Brave Search and let AI decide whether to search again or answer. Bind your Brave API key in Settings → General.',
            'web_search_section_title': 'Web Search (Brave)',
            'web_search_section_subtitle': 'Optional live web results in Ask, using your own Brave Search API key',
            'web_search_section_description': 'Turn on Web Search in the Ask toolbar to let AI look up current information. The plugin calls the Brave Search API with the key you bind here. Get a key from the <a href="https://api-dashboard.search.brave.com/">Brave Search API dashboard</a>.',
            'web_search_api_key_label': 'Brave Search API Key',
            'web_search_api_key_placeholder': 'Paste your Brave subscription token',
            'web_search_api_key_desc': 'Your key stays on this computer in the plugin settings. It is sent only to api.search.brave.com.',
            'web_search_privacy_alert': 'When Web Search is on, search queries are sent to Brave Search (not to your AI provider). Book titles in those queries may be visible to Brave.',
            'web_search_test_button': 'Test',
            'web_search_test_success': 'Brave Search API is working. Found {count} result(s) for a test query.',
            'web_search_test_failed': 'Brave Search API test failed: {error}',
            'web_search_missing_key': 'Web Search is on, but no Brave Search API key is configured. Open Configuration → General and paste your Brave API key.',
            'web_search_invalid_key': 'Brave Search rejected the API key (HTTP {status}). Check the key and your Brave plan in Settings → General.',
            'web_search_rate_limited': 'Brave Search rate limit reached (HTTP 429). Try again later or check your plan quota.',
            'web_search_request_failed': 'Web search failed for "{query}": {error}',
            'web_search_searching': 'Searching the web: {query}',
            'web_search_no_results': 'No web results for: {query}',
            ...
            'web_search_planner_prompt': "You are answering a user's question. You may request live web searches when you need current or external facts.\n\nUser question and context:\n{user_prompt}\n\nPrevious web search results:\n{search_transcript}\n\nRespond in EXACTLY one of these two formats (no extra commentary):\n\n<search>\nquery 1\nquery 2\n</search>\n\nor write the final answer in markdown. You may wrap it in <answer>...</answer>.\n\nRules:\n- If the question can be answered from the book/library context or your knowledge without current web facts, answer now.\n- If you need more information, output 1-3 focused search queries inside <search>.\n- Do not repeat a previous query.\n- After enough results, you MUST give the final answer.",
            'web_search_force_answer_prompt': 'You previously requested more web searches, but no further searches are allowed. Using only the information below, write the final markdown answer. Do not output <search> tags.\n\nUser question and context:\n{user_prompt}\n\nWeb search results:\n{search_transcript}',
