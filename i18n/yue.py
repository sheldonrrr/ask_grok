...
class CantoneseTranslation(BaseTranslation):
    ...
    def translations(self) -> dict:
        return {
            ...
            'web_search_button_tooltip': '用 Brave Search 搜全網，再由 AI 決定繼續搜定係彙總答。請先喺「設定 → 搜尋」綁定 Brave API Key。',
            'web_search_section_title': '全網搜尋（Brave）',
            'web_search_section_subtitle': '喺問詢度用你自己嘅 Brave Search API Key 攞即時網頁結果',
            'web_search_section_description': '喺問詢欄打開「全網搜尋」之後，AI 可以搜而家網上嘅資訊。插件會用你喺度綁定嘅密鑰呼叫 Brave Search API。請去 <a href="https://api-dashboard.search.brave.com/">Brave Search API 控制台</a> 申請密鑰。',
            ...
            'web_search_privacy_alert': '打開全網搜尋之後，搜尋詞會傳去 Brave Search（唔會傳去你嘅 AI 供應商）。查詢入面嘅書名等資訊 Brave 睇到。',
            ...
            'web_search_missing_key': '已經打開全網搜尋，但未設定 Brave Search API Key。請打開「設定 → 搜尋」再貼上你嘅 Brave API Key。',
            'web_search_invalid_key': 'Brave Search 拒絕咗呢個 API Key（HTTP {status}）。請喺「設定 → 搜尋」檢查密鑰同方案。',
