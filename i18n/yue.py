...
class CantoneseTranslation(BaseTranslation):
    ...
    def translations(self) -> dict:
        return {
            ...
            'web_search_button_tooltip': '用 Brave Search 搜網頁，再由 AI 決定繼續搜定係彙總答。請先喺「設定 → 一般」綁定 Brave API Key。',
            'web_search_section_title': '網頁搜尋（Brave）',
            'web_search_section_subtitle': '喺問詢度用你自己嘅 Brave Search API Key 攞即時網頁結果',
            'web_search_section_description': '喺問詢欄打開「網頁搜尋」之後，AI 可以搜而家網上嘅資訊。插件會用你喺度綁定嘅密鑰呼叫 Brave Search API。請去 <a href="https://api-dashboard.search.brave.com/">Brave Search API 控制台</a> 申請密鑰。',
            'web_search_api_key_label': 'Brave Search API Key',
            'web_search_api_key_placeholder': '貼上你嘅 Brave Subscription Token',
            'web_search_api_key_desc': '密鑰淨係保存在呢部電腦嘅插件設定，只會傳到 api.search.brave.com。',
            'web_search_privacy_alert': '打開網頁搜尋之後，搜尋詞會傳去 Brave Search（唔會傳去你嘅 AI 供應商）。查詢入面嘅書名等資訊 Brave 睇到。',
            'web_search_test_button': '測試',
            'web_search_test_success': 'Brave Search API 用得。測試查詢返咗 {count} 條結果。',
            'web_search_test_failed': 'Brave Search API 測試失敗：{error}',
            'web_search_missing_key': '已經打開網頁搜尋，但未設定 Brave Search API Key。請打開「設定 → 一般」再貼上你嘅 Brave API Key。',
            'web_search_invalid_key': 'Brave Search 拒絕咗呢個 API Key（HTTP {status}）。請喺「設定 → 一般」檢查密鑰同方案。',
            'web_search_rate_limited': 'Brave Search 已經去到速率限制（HTTP 429）。請遲啲再試，或者檢查方案配額。',
            'web_search_request_failed': '網頁搜尋失敗（「{query}」）：{error}',
            'web_search_searching': '而家搜緊：{query}',
            'web_search_no_results': '搵唔到相關網頁：{query}',
            'web_search_heading': '網頁搜尋：{query}',
            'web_search_no_previous_results': '（未有）',
            'web_search_log_planning': '而家規劃緊網頁搜尋（第 {round}/{max} 輪）…',
            'web_search_log_requested': 'AI 要求繼續搜：{queries}',
            'web_search_log_answering': 'AI 而家彙總緊答案…',
            'web_search_log_force_answer': '已經去到搜尋輪次上限，根據已有結果作答…',
