...
class JapaneseTranslation(BaseTranslation):
    ...
    def translations(self) -> dict:
        return {
            ...
            'language_preference_subtitle': 'AIの回答をインターフェース言語に合わせるかどうかを制御',
            'prompt_templates_title': 'プロンプトテンプレート',
            'prompt_templates_subtitle': '{title}、{author}、{query}などの動的フィールドを使用して、書籍情報をAIに送信する方法をカスタマイズ',
            'ask_prompts': '質問プロンプト',
            'random_questions_prompts': 'ランダム質問プロンプト',
            'multi_book_prompts_label': '複数書籍プロンプト',
            'multi_book_placeholder_hint': '書籍情報には{books_metadata}を、ユーザーの質問には{query}を使用',
            'dynamic_fields_title': '動的フィールドリファレンス',
            'dynamic_fields_subtitle': 'メアリー・シェリーの「フランケンシュタイン」から利用可能なフィールドと例の値',
            'dynamic_fields_examples': '<b>{title}</b> → Frankenstein<br><b>{author}</b> → Mary Shelley<br><b>{publisher}</b> → Lackington, Hughes, Harding, Mavor & Jones<br><b>{pubyear}</b> → 1818<br><b>{language}</b> → English<br><b>{series}</b> → (なし)<br><b>{query}</b> → あなたの質問テキスト',
            ...
            'set_default_ai_after_add_message': '「{0}」を追加しました。デフォルトAIに設定しますか？',
            'copied': 'コピーされました！',
            ...
            'model_display_name_custom': 'カスタム (OpenAI 互換)',
            'model_display_name_ollama': 'Ollama(ローカル)',
            'model_display_name_lmstudio': 'LM Studio(ローカル)',
            'model_display_name_koboldcpp': 'KoboldCpp(ローカル)',
            'custom_openai_compat_notice': 'ヒント：カスタム提供元は OpenAI Chat Completions 形式（/chat/completions）を使用します。互換性のある Base URL とモデル名を入力してください。一部のローカルサーバーでは API Key は任意です。',
            'local_openai_compat_no_api_key_notice': 'ヒント：このローカル OpenAI 互換サービスは通常 API Key を必要としません。先にローカルサーバーを起動してからモデル一覧を更新してください。',
            'lmstudio_no_api_key_notice': 'ヒント：LM Studio はローカルで OpenAI 互換 API を使用し、通常 API Key は不要です。',
            'koboldcpp_no_api_key_notice': 'ヒント：KoboldCpp はローカルで OpenAI 互換 API を使用し、通常 API Key は不要です。',
            'local_service_not_running': 'ローカル AI サービスに接続できません。起動中であることと Base URL が正しいことを確認してください。',
            ...
            'base_url_desc_kimi': '国際版のキーは https://api.moonshot.ai/v1、中国版のキーは https://api.moonshot.cn/v1 を使用します。混在させないでください。',
            'kimi_region_label': 'プラットフォーム',
            'kimi_region_global': '国際版',
            'kimi_region_china': '中国本土',
            'kimi_base_url_readonly_tip': 'Base URL は選択したプラットフォームによって決まります。',
