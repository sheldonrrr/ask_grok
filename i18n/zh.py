...
class SimplifiedChineseTranslation(BaseTranslation):
    ...
    def translations(self) -> dict:
        return {
            ...
            'set_default_ai_after_add_message': '已成功添加「{0}」。是否将其设为默认 AI？',
            ...
            'model_display_name_custom': '自定义 (OpenAI 兼容)',
            'model_display_name_ollama': 'Ollama(本地)',
            'model_display_name_lmstudio': 'LM Studio(本地)',
            'model_display_name_koboldcpp': 'KoboldCpp(本地)',
            'custom_openai_compat_notice': '提示：自定义提供商使用 OpenAI Chat Completions 请求格式（/chat/completions）。请填写兼容的 Base URL 与模型名称。部分本地服务可不填 API Key。',
            'local_openai_compat_no_api_key_notice': '提示：此本地 OpenAI 兼容服务通常不需要 API Key。请先启动本地服务，再刷新模型列表。',
            'lmstudio_no_api_key_notice': '提示：LM Studio 本地使用 OpenAI 兼容接口，通常不需要 API Key。',
            'koboldcpp_no_api_key_notice': '提示：KoboldCpp 本地使用 OpenAI 兼容接口，通常不需要 API Key。',
            'local_service_not_running': '无法连接本地 AI 服务。请确认服务已启动且 Base URL 正确。',
            ...
            'ollama_no_api_key_notice': '提示：Ollama 本地使用 OpenAI 兼容接口，通常不需要 API Key。',
            ...
            'web_search_button': '网页搜索',
            'web_search_button_tooltip': '使用 Brave Search 检索全网，并由 AI 决定继续搜索或汇总回答。请先在「设置 → 通用」中绑定 Brave API Key。',
            'web_search_section_title': '网页搜索（Brave）',
            'web_search_section_subtitle': '在问询中使用你自己的 Brave Search API Key 获取实时网页结果',
            'web_search_section_description': '在问询栏打开「网页搜索」后，AI 可以检索当前网络信息。插件会使用你在此绑定的密钥调用 Brave Search API。请到 <a href="https://api-dashboard.search.brave.com/">Brave Search API 控制台</a> 申请密钥。',
            'web_search_api_key_label': 'Brave Search API Key',
            'web_search_api_key_placeholder': '粘贴你的 Brave Subscription Token',
            'web_search_api_key_desc': '密钥只保存在本机的插件设置中，仅会发送到 api.search.brave.com。',
            'web_search_privacy_alert': '打开网页搜索后，搜索词会发送给 Brave Search（不会发给你的 AI 供应商）。查询中的书名等信息对 Brave 可见。',
            'web_search_test_button': '测试',
            'web_search_test_success': 'Brave Search API 可用。测试查询返回了 {count} 条结果。',
            'web_search_test_failed': 'Brave Search API 测试失败：{error}',
            'web_search_missing_key': '已打开网页搜索，但尚未配置 Brave Search API Key。请打开「配置 → 通用」并粘贴你的 Brave API Key。',
            'web_search_invalid_key': 'Brave Search 拒绝了该 API Key（HTTP {status}）。请在「设置 → 通用」中检查密钥和套餐。',
            'web_search_rate_limited': 'Brave Search 已达到速率限制（HTTP 429）。请稍后再试，或检查套餐配额。',
            'web_search_request_failed': '网页搜索失败（“{query}”）：{error}',
            'web_search_searching': '正在搜索：{query}',
            'web_search_no_results': '没有找到相关网页：{query}',
            'web_search_heading': '网页搜索：{query}',
            'web_search_no_previous_results': '（尚无）',
            'web_search_log_planning': '正在规划网页搜索（第 {round}/{max} 轮）…',
            'web_search_log_requested': 'AI 请求继续搜索：{queries}',
            'web_search_log_answering': 'AI 正在汇总回答…',
            'web_search_log_force_answer': '已达搜索轮次上限，根据已有结果作答…',
