...
class SimplifiedChineseTranslation(BaseTranslation):
    ...
    def translations(self) -> dict:
        return {
            ...
            'web_search_button': '全网搜索',
            'web_search_button_tooltip': '使用 Brave Search 检索全网，并由 AI 决定继续搜索或汇总回答。请先在「设置 → 搜索」中绑定 Brave API Key。',
            'web_search_section_title': '全网搜索（Brave）',
            'web_search_section_subtitle': '在问询中使用你自己的 Brave Search API Key 获取实时网页结果',
            'web_search_section_description': '在问询栏打开「全网搜索」后，AI 可以检索当前网络信息。插件会使用你在此绑定的密钥调用 Brave Search API。请到 <a href="https://api-dashboard.search.brave.com/">Brave Search API 控制台</a> 申请密钥。',
            ...
            'web_search_privacy_alert': '打开全网搜索后，搜索词会发送给 Brave Search（不会发给你的 AI 供应商）。查询中的书名等信息对 Brave 可见。',
            ...
            'web_search_missing_key': '已打开全网搜索，但尚未配置 Brave Search API Key。请打开「配置 → 搜索」并粘贴你的 Brave API Key。',
            'web_search_invalid_key': 'Brave Search 拒绝了该 API Key（HTTP {status}）。请在「设置 → 搜索」中检查密钥和套餐。',
