...
class RussianTranslation(BaseTranslation):
    ...
    def translations(self) -> dict:
        return {
            ...
            'http_status_detail': 'HTTP {status} для URL: {url}',
            ...
            'about_latest_update': 'Последнее обновление (2026.09.02): исправлена недоступность бесплатного ИИ-сервиса',
            ...
            'free_tier_gone': 'Бесплатный канал Nvidia больше недоступен (HTTP 410). Настройте собственный ключ API Nvidia в Параметры → AI (Nvidia AI) или повторите попытку позже, если бесплатный сервис будет восстановлен.',
            ...
            'library_update_tooltip': 'Проиндексировать названия и авторов и пересобрать компактный кэш промпта',
            ...
            'ai_search_welcome_message': (
                'ИИ-поиск включён. Ищите в библиотеке на естественном языке (название + автор).\n'
                '\n'
                'Откройте Ask без выбора книг или Инструменты → ИИ-поиск.\n'
                'После добавления/удаления книг обновите данные библиотеки, чтобы обновить кэш промпта.'
            ),
            ...
            'ai_search_feature_description': (
                'Ищите в библиотеке на естественном языке (название + автор).\n'
                'Откройте Ask без выбранных книг или Инструменты → ИИ-поиск.\n'
                'Нажмите «Обновить данные библиотеки» после изменений — это обновит компактный кэш промпта.'
            ),
            'ai_search_usage_hint': '',
            'ai_search_data_title': 'Индекс библиотеки',
            'ai_search_data_subtitle': 'Пересобрать индекс и кэш промпта после добавления или удаления книг',
            'library_search_system_message': 'Вы помощник поиска по библиотеке calibre. Отвечайте ТОЛЬКО списком совпадающих книг в запрошенном HTML-формате. Без анализа, без цепочки рассуждений, без предисловия.',
