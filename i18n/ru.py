...
class RussianTranslation(BaseTranslation):
    ...
    def translations(self) -> dict:
        return {
            ...
            # Вкладка промптов
            'prompts_explanation_title': 'Как работают промпты',
            'prompts_explanation': 'Когда вы нажимаете Отправить, плагин извлекает и объединяет динамические поля из вашего шаблона промпта, затем отправляет их ИИ. Основной принцип заключается в том, что ИИ опирается на данные обучения, которые включают информацию о книге, а не отправляет полный текст книги ИИ.',
