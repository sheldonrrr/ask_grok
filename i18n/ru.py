...
class RussianTranslation(BaseTranslation):
    ...
    def translations(self) -> dict:
        return {
            ...
            'prompts_subtitle': 'Настройте способ отправки вопросов к ИИ',
            'export_settings_subtitle': 'Установите папку по умолчанию для экспорта PDF',
            'debug_settings_subtitle': 'Включите журналирование отладки для устранения неполадок',
            ...
            'language_preference_subtitle': 'Управляйте соответствием ответов ИИ языку вашего интерфейса',
            'prompt_templates_title': 'Шаблоны Промптов',
            'prompt_templates_subtitle': 'Настройте отправку информации о книге в ИИ с помощью динамических полей, таких как {title}, {author}, {query}',
            'ask_prompts': 'Промпты Вопросов',
            'random_questions_prompts': 'Промпты Случайных Вопросов',
            'multi_book_prompts_label': 'Промпты Нескольких Книг',
            'multi_book_placeholder_hint': 'Используйте {books_metadata} для информации о книгах, {query} для вопроса пользователя',
            ...
            'dynamic_fields_examples': '<b>{title}</b> → Frankenstein<br><b>{author}</b> → Mary Shelley<br><b>{publisher}</b> → Lackington, Hughes, Harding, Mavor & Jones<br><b>{pubyear}</b> → 1818<br><b>{language}</b> → English<br><b>{series}</b> → (нет)<br><b>{query}</b> → Текст вашего вопроса',
            'reset_prompts': 'Сбросить промпты по умолчанию',
            'reset_prompts_confirm': 'Вы уверены, что хотите сбросить все шаблоны промптов к значениям по умолчанию? Это действие нельзя отменить.',
            'unsaved_changes_title': 'Несохраненные изменения',
            'unsaved_changes_message': 'У вас есть несохраненные изменения во вкладке Промпты. Хотите сохранить их?',
            ...
            'enable_debug_logging': 'Включить журнал отладки (ask_ai_plugin_debug.log)',
            'debug_logging_hint': 'При отключении журналы отладки не будут записываться в файл. Это может предотвратить чрезмерное увеличение размера файла журнала.',
