...
class RussianTranslation(BaseTranslation):
    ...
    def translations(self) -> dict:
        return {
            ...
            'set_default_ai_after_add_message': '«{0}» успешно добавлен. Установить его как ИИ по умолчанию?',
            ...
            'model_display_name_custom': 'Пользовательский (совместим с OpenAI)',
            'model_display_name_ollama': 'Ollama(Локальный)',
            'model_display_name_lmstudio': 'LM Studio(Локальный)',
            'model_display_name_koboldcpp': 'KoboldCpp(Локальный)',
            'custom_openai_compat_notice': 'Примечание: Custom использует формат OpenAI Chat Completions (/chat/completions). Укажите совместимые Base URL и имя модели. API Key для некоторых локальных серверов необязателен.',
            'local_openai_compat_no_api_key_notice': 'Примечание: этот локальный OpenAI-совместимый сервис обычно не требует API Key. Запустите локальный сервер, затем обновите список моделей.',
            'lmstudio_no_api_key_notice': 'Примечание: LM Studio локально использует OpenAI-совместимый API и обычно не требует API Key.',
            'koboldcpp_no_api_key_notice': 'Примечание: KoboldCpp локально использует OpenAI-совместимый API и обычно не требует API Key.',
            'local_service_not_running': 'Не удаётся подключиться к локальному AI-сервису. Убедитесь, что он запущен и Base URL указан верно.',
            ...
            'base_url_desc_kimi': 'Международные ключи используют https://api.moonshot.ai/v1; ключи китайской платформы — https://api.moonshot.cn/v1. Не смешивайте их.',
            'kimi_region_label': 'Платформа',
            'kimi_region_global': 'Международная',
            'kimi_region_china': 'Китай (материковый)',
            'kimi_base_url_readonly_tip': 'Базовый URL определяется выбранной платформой.',
