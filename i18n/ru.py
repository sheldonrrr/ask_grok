...
class RussianTranslation(BaseTranslation):
    ...
    def default_template(self) -> str:
        return '''Контекст: Вы помогаете пользователю calibre (http://calibre-ebook.com), мощного приложения для управления электронными книгами, через плагин "Ask AI Plugin". Этот плагин позволяет пользователям задавать вопросы о книгах в их библиотеке calibre. Примечание: Этот плагин может отвечать только на вопросы о содержании, темах или связанных темах выбранной книги - он не может напрямую изменять метаданные книги или выполнять операции calibre.
...
Пожалуйста, предоставьте полезный ответ на основе информации о книге и ваших знаний.'''
